...
pub fn read(parser: *parse.Parser, start: parse.EntityStart) common.ReadError!AttDef {
    ...
    while (try parser.next()) |event| {
        switch (event) {
            .pair => |token| try attdef.apply(token),
            .entity_end => return attdef,
...
test "read ATTDEF entity" {
    ...
    const attdef = try AttDef.read(&parser, start);
    try std.testing.expectEqual(.attdef, start.kind);
    try std.testing.expectEqualStrings("Anno", attdef.layer.?);
    try std.testing.expectEqualStrings("ID", attdef.tag.?);
    try std.testing.expectEqualStrings("Identifier", attdef.prompt.?);
    try std.testing.expectEqualStrings("A-000", attdef.default_value.?);
    try std.testing.expectEqual(@as(f64, 1), attdef.start.x);
    try std.testing.expectEqual(@as(f64, 2), attdef.start.y);
    try std.testing.expectEqual(@as(f64, 3), attdef.height);
    try std.testing.expectEqual(@as(i16, 2), attdef.flags);
}
